...
isfile(depfile) || error("JSON not properly installed. Please run Pkg.build(\"JSON\")")
include(depfile) # generated by Pkg.build("JSON")
if _HAVE_DATASTRUCTURES # in deps.jl
...
function parse(str::AbstractString; ordered::Bool=false)
    ...
    len::Int = endof(str)
    len < 1 && return
    ordered && !_HAVE_DATASTRUCTURES && error("DataStructures package required for ordered parsing: try `Pkg.add(\"DataStructures\"); Pkg.build(\"JSON\")`") 
